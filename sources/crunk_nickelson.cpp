...
CrunkNickelson::CrunkNickelson(int n, double eps, double time_start, double time_target): Solver(n, eps, time_start, time_target) {
	TAU = 0.1;
}
	
	
void CrunkNickelson::iterationStep(int n) {
	...
	/* Для схемы Кранка-Никольсона получается СЛАУ с трехдиагональной матрицей, которую можно
...
double CrunkNickelson::b(int n) {
	
	int border = (int) floor(phase_front_coord / H) + 1;
	if (n < border)
		return -2.0 - 2 * H * H / k_1 / TAU;
	else return -2.0 - 2 * H * H / k_2 / TAU;
	
}
	
double CrunkNickelson::d(int n) {
	
	int border = (int) floor(phase_front_coord / H) + 1;
	if (n < border)
		return (-2.0) * H * H / k_1 / TAU * time_layers.back()[n] -\
			(time_layers.back()[n - 1] - 2 * time_layers.back()[n] + time_layers.back()[n + 1]);
	else return (-2.0) * H * H / k_2 / TAU * time_layers.back()[n] -\
		(time_layers.back()[n - 1] - 2 * time_layers.back()[n] + time_layers.back()[n + 1]);
}
